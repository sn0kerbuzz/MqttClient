//
// Created by Andrei Balasescu on 12/04/2020.
//
...
void handleMqttConnected() {
	...
	client.subscribe("example/topic1", [] (const String& topic, const String& payload) {
		Serial.println("Message received on example/topic1");
		Serial.println(payload);
	});

	client.subscribe("example/topic2", [] (const String& topic, const String& payload) {
		Serial.println("Message received on example/topic2");
		Serial.println(payload);
	});
}
...
void handleMqttMessageReceived(const String& topic, const String& payload) {
	...
	Serial.println("Message received on global callback");
